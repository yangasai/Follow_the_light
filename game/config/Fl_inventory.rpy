...
screen Fl_inventory(type=None, item=None):
    ...
    fixed offset (27, -4) at Fl_menu_move:
        frame background "gui/inventory/icons/" + Fl_hover_slot["name"] + ".png"
        frame background "gui/inventory/background.png"

        imagebutton offset (1378, 279):
            idle "gui/inventory/slots/" + Fl_backpack["name"] + "_idle.png"
            if Fl_backpack["name"] != "empty":
                hover_sound "gui/main_menu/Fl_click_menu.mp3"
                hover Fl.Brightness("gui/inventory/slots/" + Fl_backpack["name"] + "_hover.png", 0.1)
                insensitive Fl.Brightness("gui/inventory/slots/" + Fl_backpack["name"] + "_idle.png", 0.5)
                action Fl_SelectedSlot("BPK")

        imagebutton offset (1378, 363):
            idle "gui/inventory/slots/" + Fl_weapon["name"] + "_idle.png"
            if Fl_weapon["name"] not in ["empty", "lock"]:
                hover_sound "gui/main_menu/Fl_click_menu.mp3"
                hover Fl.Brightness(+ "gui/inventory/slots/" + Fl_weapon["name"] + "_idle.png", 0.1)
                insensitive Fl.Brightness(+ "gui/inventory/slots/" + Fl_weapon["name"] + "_idle.png", 0.5)
        ...
        for slot in range(0, Fl.InventorySlotsLen()):
            imagebutton:
                ...
                if Fl_inventory[slot]["name"] != "lock" and slot > 3:
                    foreground "gui/inventory/slots/plus_idle.png"
                    hover_foreground "gui/inventory/slots/plus_hover.png"
                    if Fl_inventory[slot]["name"] != "empty":
                        insensitive_foreground Fl.Brightness("gui/inventory/slots/plus_idle.png", 0.5)
